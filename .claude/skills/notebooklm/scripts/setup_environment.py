...
class SkillEnvironment:
    ...
    def ensure_venv(self) -> bool:
        ...
        if self.is_in_skill_venv():
            print("[V] Already running in skill virtual environment")
            return True
        ...
        if self.requirements_file.exists():
            ...
            try:
                # Upgrade pip first (skipping on Windows to avoid access errors)
                # subprocess.run(
...
def main():
    ...
    if args.check:
        if env.venv_dir.exists():
            print(f"[V] Virtual environment exists: {env.venv_dir}")
