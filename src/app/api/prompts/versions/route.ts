...
export async function GET(request: Request) {
  ...
  const { data, error } = await getSupabaseAdmin().from('prompt_versiones').select('*').eq('agente', agente).order('creado_en', { ascending: false }).limit(10)
...
export async function POST(request: Request) {
  ...
  const { data: version, error } = await getSupabaseAdmin().from('prompt_versiones').select('contenido').eq('id', id).single()
